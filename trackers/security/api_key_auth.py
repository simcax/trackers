...
def init_security(app, api_key_service=None):
    ...
    # Skip registering before_request handler - unified authentication system handles this
    # The unified authentication system in trackers/auth/decorators.py provides
    # more comprehensive authentication handling with both API key and Google OAuth support
    app.logger.info(
        "Skipping legacy API key before_request handler - using unified authentication system"
    )
